# Hearthmon — Everything to Try ✦

A fuller tour than the quickstart. Poke around — a lot of this is subtle *on purpose*
(presence over noise). Here's what's actually in there.

## 🌿 The soul (the real point)
...
## 🎮 The fun
- **Full Pokédex (all 1025)** — switch companion by search, type & generation filters, a random
  🎲 picker, or an auto-switch timer.
- **1 / 128 shiny odds** on switches. ✨
- **Ash throw ceremony** — recall beam → trainer winds up → *"<Name>, go!"* → the ball arcs in
...
**No — not by default.** You re-run the exe yourself. If you want it on every login, flip
**"Start with Windows"** in settings; then it launches at boot (and asks before popping up, so
it's never intrusive). Turn it off there anytime.

## 🔒 Privacy
100% local. No account, no server, nothing uploaded — ever. With awareness on, it sees process
**names** only; never titles, keystrokes, screen, code, or audio.
